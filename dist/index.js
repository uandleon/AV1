...
const TipoAeronautico_1 = require("./enums/TipoAeronautico");
...
0. Sair
  `);
    const opcao = (0, input_1.perguntar)("Escolha uma opcao: ");
    switch (opcao) {
        case "1":
            login();
            break;
        ...
        case "3":
            verificarLogin(cadastrarAeronave);
            break;
        case "4":
            verificarLogin(listarAeronaves);
            break;
        case "5":
            verificarLogin(cadastrarPeca);
            break;
        case "7":
            verificarLogin(cadastrarEtapa);
            break;
        case "8":
            verificarLogin(listarEtapas);
            break;
        case "9":
            verificarLogin(associarFuncionarioEtapa);
            break;
        case "10":
            verificarLogin(listarFuncionariosEtapa);
            break;
        case "11":
            verificarLogin(iniciarEtapa);
            break;
        case "12":
            verificarLogin(finalizarEtapa);
            break;
        case "13":
            verificarLogin(cadastrarResultadoTeste);
            break;
        case "14":
            verificarLogin(listarEtapasComResultados);
            break;
        case "0":
...
function cadastrarFuncionario() {
    ...
    const nome = (0, input_1.perguntar)("Nome: ");
    const email = (0, input_1.perguntar)("Email: ");
    const senha = (0, input_1.perguntar)("Senha: ");
    console.log("Permissões disponíveis:");
    Object.values(NivelPermissao_1.NivelPermissao).forEach((p, i) => console.log(`${i + 1}. ${p}`));
    const permissaoIndex = parseInt((0, input_1.perguntar)("Escolha a permissão (1-3): ")) - 1;
    const permissao = Object.values(NivelPermissao_1.NivelPermissao)[permissaoIndex];
    if (!permissao) {
...
function cadastrarAeronave() {
    ...
    const modelo = (0, input_1.perguntar)("Modelo: ");
    const fabricante = (0, input_1.perguntar)("Fabricante: ");
    console.log("Tipos de Aeronave disponíveis:");
    Object.values(TipoAeronautico_1.TipoAeronautico).forEach((t, i) => console.log(`${i + 1}. ${t}`));
    const tipoIndex = parseInt((0, input_1.perguntar)("Escolha o tipo (1-2): ")) - 1;
    const tipo = Object.values(TipoAeronautico_1.TipoAeronautico)[tipoIndex];
    if (!tipo) {
...
function cadastrarPeca() {
    console.log("\n--- Cadastro de Peça ---");
    const nome = (0, input_1.perguntar)("Nome da peça: ");
...
function cadastrarEtapa() {
    ...
    aeronaves.forEach(a => console.log(a.descrever()));
    const aeronaveId = Number((0, input_1.perguntar)("Digite o ID da aeronave: "));
    const aeronave = aeronaveController.buscarPorId(aeronaveId);
    if (!aeronave) {
        console.log("Aeronave não encontrada.");
        return;
    }
    const nomeEtapa = (0, input_1.perguntar)("Nome da etapa: ");
    const descricao = (0, input_1.perguntar)("Descrição (opcional): ");
...
function associarFuncionarioEtapa() {
    ...
    etapas.forEach(e => console.log(e.descrever()));
    const etapaId = Number((0, input_1.perguntar)("Digite o ID da etapa: "));
    ...
    const funcionarioId = Number((0, input_1.perguntar)("Digite o ID do funcionário: "));
...
function cadastrarResultadoTeste() {
    ...
    Object.values(ResultadoTeste_1.ResultadoTeste).forEach((r, i) => console.log(`${i + 1}. ${r}`));
    const resultadoIndex = Number((0, input_1.perguntar)("Escolha o resultado: ")) - 1;
    const resultado = Object.values(ResultadoTeste_1.ResultadoTeste)[resultadoIndex];
    if (!resultado) {
...
function listarEtapasComResultados() {
    ...
    const etapasComResultado = etapas.filter(e => e.resultadoTeste);
